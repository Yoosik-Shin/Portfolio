...
export const TrdMenu = () => {
  ...
  const handleOpen = (project) => {
    ...
    if (project.title === "반려동물 커뮤니티 (zootopia_react)") {
      ...
      setPageType('The Joyful Delivery');
      setShowFullPage(true);
    }
};
    ...
    <>
      <div className='relative w-[1480px] h-full top-[100px] flex flex-col justify-center items-center gap-8'>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
          ...
          {/* 전용 페이지 다이얼로그 */}
    <Dialog open={showFullPage} onClose={handleCloseFullPage} fullWidth maxWidth={false}>
      <button onClick={handleCloseFullPage} className="absolute top-4 right-4">닫기</button>
      {pageType === 'zootopia_react' && <ZootopiaProject />}
      {pageType === 'zootopia_original' && <ZootopiaOriginalProject />}
      {pageType === 'joyful_delivery' && <JoyfulDeliveryProject />}
    </Dialog>
